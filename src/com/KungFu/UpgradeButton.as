package com.KungFu
{
	 ...
	 */
	public class UpgradeButton extends FlxCore
	{
		/**
		...
		/**
		 * This function is called when the button moused over.
		 */		
		protected var _callback2:Function;
		/**
		 ...
		 */
		public function UpgradeButton(X:int,Y:int,Callback:Function,Callback2:Function,PassVar:int=-1)
		{
			super();
		 ...
		 */
		public function loadGraphic(Image:FlxSprite,ImageHighlight:FlxSprite=null):UpgradeButton
		{
			_off = Image;
			_off.scrollFactor = scrollFactor;
		 ...
		 * @return	This FlxButton instance (nice for chaining stuff together, if you're into that).
		 */
		public function loadText(Text:FlxText,TextHighlight:FlxText=null):UpgradeButton
		{
		...
		{
			if(On || _onToggle) //If on toggle always leave on sprite showing -- Lupi
			{
				_off.visible = false;
		...
		protected function onMouseUp(event:MouseEvent):void
		{
			...
			if ( _off.overlapsPoint(FlxG.mouse.x + (1 - scrollFactor.x) * FlxG.scroll.x, FlxG.mouse.y + (1 - scrollFactor.y) * FlxG.scroll.y) && _pass_var == -1) 
			{
				_callback();
				return;
			}
