package com.KungFu
{
    ...
    public class Player extends FlxSprite
    {
		...
		private var jump_power:int = 900;	
		...
		private function attacks():void
		{
			...
			if(FlxG.keys.justPressed("C")  && _attack_counter <= 0 && _throw_counter <= 0 )
			{
			...
			//POWER PUNCH!!!
			if (FlxG.keys.LEFT == false && FlxG.keys.RIGHT == false && FlxG.keys.pressed("C")  && _attack_counter <= 0 && _throw_counter <= 0 && jump == 0 && Globals.global_power[2] == 1)
			{
				if (_power_punch == 0)
				{
					yell_sound.stop();
		...
		private function movement():void
		{
			...
			//Double Jumping
			if(FlxG.keys.justPressed("X") && _double_jump == 0 && Globals.global_power[0] == 1 && velocity.y != 0)
			{
				velocity.y = -150;
				FlxG.play(SndJump);
				_double_jump = 1;	
			...
			if(!FlxG.keys.X && velocity.y < 0 && _attack_counter <= 0)
			{
				velocity.y += 8;
			}				
		}
		...
		override public function hitFloor(Contact:FlxCore=null):Boolean
		{
			if (Contact is FlxBlockJumpThrough )
			{
				...
				if ( (FlxG.keys.DOWN && FlxG.keys.pressed("X") ) || y + height >= Contact.y + 8 )
				{
					return false;
				}
				jump_through = true;
